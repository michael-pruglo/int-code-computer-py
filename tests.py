...
class IntCodeTest(unittest.TestCase):
    ...
    def test_opcode8p(self):
        for val in [4, 8, 10]:
            ic = IntCode()
            ic.set_mem([3,9,8,9,10,9,4,9,99,-1,8])
            ic.set_input([val])
            ic.exec()
            self.assertEqual([int(val==8)], ic.get_output())

    def test_opcode8i(self):
        for val in [1, 8, 14]:
            ic = IntCode()
            ic.set_mem([3,3,1108,-1,8,3,4,3,99])
            ic.set_input([val])
            ic.exec()
            self.assertEqual([int(val==8)], ic.get_output())

    def test_opcodestill8(self):
        for val in [0, 8, 19]:
            ic = IntCode()
            ic.set_mem([3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99])
            ic.set_input([val])
            ic.exec()
            self.assertEqual([999 if val<8 else 1000 if val==8 else 1001], ic.get_output())
    ...
    def test_day3(self):
        ...
        ic.set_input([5])
        ic.exec()
        self.assertEqual([7704130], ic.get_output())
